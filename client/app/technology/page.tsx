  ...
  return (
    <div className="flex flex-col bg-black text-white px-6">
      ...
      <section className="h-[100dvh] flex flex-col items-center justify-center relative overflow-hidden bg-[#0E0E10]">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(140,107,79,0.05)_0%,transparent_70%)] pointer-events-none" />
        <div className="relative z-10 text-center max-w-5xl px-6">
          ...
          <motion.h1 {...fadeUp(0.05)} className="text-4xl md:text-8xl lg:text-9xl font-black tracking-tighter mb-8 md:mb-10 leading-[0.9]">
            Quantum <br />
            <span className="text-white/20 transition-colors hover:text-white">Core Protocol.</span>
          </motion.h1>
      ...
      <section className="min-h-[100dvh] py-20 flex items-center bg-white border-t-[1px] border-accent/20 relative overflow-hidden">
        <div className="container-custom grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-20 items-center px-6 md:px-12 mx-auto">
          <motion.div {...fadeUp(0)} className="text-black">
            <div className="w-12 h-12 md:w-16 md:h-16 rounded-2xl md:rounded-3xl bg-black/[0.03] flex items-center justify-center text-black/20 mb-8 md:mb-10">
              <Network size={28} strokeWidth={1} />
            </div>
            <h2 className="text-3xl md:text-6xl font-black tracking-tighter mb-6 md:mb-8 leading-tight">Neural Mesh <br /> <span className="text-black/20">Fabric.</span></h2>
            <p className="text-base md:text-lg text-black/40 leading-relaxed mb-10 md:mb-12 max-w-md font-medium">
            ...
            <div className="space-y-6">
              {[
                ...
                <div key={item.title} className="p-6 bg-black/[0.02] border border-black/[0.05] rounded-2xl hover:border-accent/40 transition-colors group">
                  <div className="text-[10px] font-black uppercase tracking-widest text-black/20 mb-2 group-hover:text-accent transition-colors">{item.title}</div>
                  <div className="text-sm font-medium text-black/60">{item.detail}</div>
                </div>
              ))}
            </div>
          </motion.div>
        ...
        <motion.div {...fadeUp(0)} className="max-w-4xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 mb-16 md:mb-20">
             {[
               ...
               <div key={stat.label} className="p-8 md:p-10 bg-white/[0.01] border border-white/5 rounded-[24px] md:rounded-[32px] hover:bg-white/[0.03] transition-all">
                  <stat.icon className="mx-auto mb-4 md:mb-6 text-white/20" size={28} strokeWidth={1} />
                  <div className="text-2xl md:text-3xl font-bold tracking-tighter mb-2">{stat.val}</div>
                  <div className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] text-white/20">{stat.label}</div>
               </div>
             ))}
          </div>
